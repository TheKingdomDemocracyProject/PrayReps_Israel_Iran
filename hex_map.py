...
def plot_hex_map_with_hearts(
    ...
    logging.debug(
        f"Plotting hex map for country {country_code}. Prayed: {len(prayed_for_items_list)}, Queue: {len(queue_items_list)}"
    ...
    if hex_map_gdf is None or hex_map_gdf.empty:
        ...
        try:
            plt.savefig(
                output_path_for_error, bbox_inches="tight", pad_inches=0.5, dpi=100
            )
            logging.info(
                f"Saved placeholder map to {output_path_for_error} due to missing map data for {country_code}."
            )
    ...
    if os.path.exists(output_path):
        try:
            ...
            logging.debug(
                f"File {output_path} exists. Last modified: {time.ctime(mod_time)} (Timestamp: {mod_time})"
            )
    ...
    if country_code in ["israel", "iran"] and (
        ...
        if "id" not in hex_map_gdf.columns:
            logging.error(
                f"'id' column missing in hex_map_gdf for {country_code}. Cannot apply random allocation. Saving base map."
            )
            fig_base = None
            try:
                ...
                logging.info(
                    f"Saved base map for {country_code} without hearts/highlights due to missing 'id' column."
                )
    ...
    try:
        ...
        logging.info(
            f"Plotting map for country: {country_code}. Prayed items: {len(prayed_for_items_list)}, Queue items: {len(queue_items_list)}"
        )
        ...
        if country_code in ["israel", "iran"]:
            ...
            logging.debug(
                f"IR/ISR: Processing {len(prayed_for_items_list)} prayed items for heart placement using assigned hex_id."
            )
                ...
                if assigned_hex_id:
                    ...
                    if not location_geom.empty:
                        ...
                        if heart_img:
                            ...
                            logging.warning(
                                f"Skipping heart for hex ID {assigned_hex_id} in {country_code} (image load failed). Item: {prayed_item.get('person_name')}"
                            )
                    else:
                        logging.warning(
                            f"Geometry not found for assigned hex ID {assigned_hex_id} in {country_code}. Cannot place heart. Item: {prayed_item.get('person_name')}"
                        )
                else:
                    ...
                    logging.warning(
                        f"Prayed item {prayed_item.get('person_name')} for {country_code} (random alloc) has no assigned hex_id. Heart not placed."
                    )
            logging.debug(
                f"IR/ISR: Placed {placed_heart_count} hearts based on assigned hex_ids."
            )
        else:
            # Specific Mapping Strategy (remains the same)
            if post_label_mapping_df is None or post_label_mapping_df.empty:
                logging.error(
                    f"Post label mapping is missing or empty for {country_code}. Cannot map items to hexes."
                )
            elif not all(
                ...
                for location_label in prayed_locations_labels:
                    location_code_series = post_label_mapping_df.loc[
                        post_label_mapping_df["post_label"] == location_label, "name"
                    ]
                    if not location_code_series.empty:
                        location_code = location_code_series.iloc[0]
                        if "name" not in hex_map_gdf.columns:
                            logging.error(
                                f"'name' column missing in hex_map_gdf for {country_code}. Cannot map by name."
                            )
                            continue
                        ...
                        if not location_geom.empty:
                            ...
                            if heart_img:
                                ...
                                logging.warning(
                                    f"Skipping heart placement for a hex in {country_code} due to missing heart image."
                                )
                        else:
                            logging.debug(
                                f"No geometry found for location code {location_code} (from label {location_label}) in {country_code}."
                            )
                    else:
                        logging.debug(
                            f"No mapping found for post_label {location_label} in {country_code}."
                        )
        ...
        if queue_items_list:
            ...
            if top_queue_item.get("country_code") == country_code:
                ...
                if country_code in ["israel", "iran"]:
                    ...
                    if (
                        "id" in hex_map_gdf.columns
                    ):  # Should be true due to earlier check
                        num_hearts_already_plotted = len(prayed_for_items_list)
                        all_hex_ids_list = list(
                            hex_map_gdf["id"].unique()
                        )  # All unique IDs from the map
                        ...
                        logging.debug(
                            f"For {country_code} highlight: All map IDs: {len(all_hex_ids_set)}, Prayed (heart) IDs: {len(prayed_hex_ids_set)}, Available for highlight: {len(available_ids_for_highlight)}"
                        )
                        ...
                        if assigned_hex_id_for_highlight:
                            logging.info(
                                f"Attempting to highlight pre-assigned hex ID {assigned_hex_id_for_highlight} for queue item in {country_code}."
                            )
                            ...
                            if not location_geom_q.empty:
                                ...
                                if geom.geom_type == "Polygon":
                                    ...
                                    for poly_geom in list(
                                        ...
                                        ax.add_patch(hex_patch)
                                logging.info(
                                    f"Successfully highlighted pre-assigned hex ID {assigned_hex_id_for_highlight} for {country_code}."
                                )
                            else:
                                logging.warning(
                                    f"Highlight failed for {country_code}: Pre-assigned hex ID {assigned_hex_id_for_highlight} found NO GEOMETRY in map data. Falling back to random if possible."
                                )
                            ...
                            logging.info(
                                f"No pre-assigned hex_id for top queue item in {country_code}. Attempting dynamic random highlight."
                            )
                            ...
                            if available_ids_for_highlight_fallback:
                                ...
                                logging.info(
                                    f"Dynamically selected random hex ID {hex_id_to_highlight_fallback} for queue highlight in {country_code} from {len(available_ids_for_highlight_fallback)} available hexes."
                                )
                                ...
                                if not location_geom_q_fallback.empty:
                                    ...
                                    if geom_fallback.geom_type == "Polygon":
                                        ...
                                        for poly_fallback in list(geom_fallback.geoms):
                                            ...
                                            ax.add_patch(hex_patch)
                                    logging.info(
                                        f"Highlighted hex {hex_id_to_highlight_fallback} for {country_code} (dynamic random strategy)."
                                    )
                                else:
                                    logging.warning(
                                        f"Dynamic highlight failed for {country_code}: Randomly selected hex ID {hex_id_to_highlight_fallback} found NO GEOMETRY in map data."
                                    )
                            else:
                                logging.info(
                                    f"All hexes already prayed for (or have assigned hex_ids) in {country_code}, nothing to highlight for queue (dynamic random strategy)."
                                )
                else:
                            ...
                            if not location_code_series_q.empty:
                                ...
                                if not location_geom_q.empty:
                                    ...
                                    if geom.geom_type == "Polygon":
                                        ...
                                        ax.add_patch(hex_patch)
                                        logging.info(
                                            f"Highlighted hex for {top_queue_post_label} in {country_code} (specific strategy)."
                                        )
                                    elif geom.geom_type == "MultiPolygon":
                                        for poly in list(
                                            geom.geoms
                                            ...
                                            ax.add_patch(hex_patch)
                                        logging.info(
                                            f"Highlighted (multi)hex for {top_queue_post_label} in {country_code} (specific strategy)."
                                        )
                                else:
                                    logging.warning(
                                        f"No geometry for location code {location_code_q} in {country_code} for specific queue highlighting."
                                    )
                            else:
                                logging.warning(
                                    f"No mapping found for post_label {top_queue_post_label} in {country_code} for specific queue highlighting."
                                )
                        else:
                            logging.info(
                                f"Top queue item for {country_code} has no post_label for specific highlighting."
                            )
                    else:
                        logging.warning(
        ...
        try:
            ...
            logging.info(
                f"Saved error placeholder map for {country_code} due to plotting exception."
            )
    ...
    if os.path.exists(output_path):
        try:
            ...
            logging.debug(
                f"File {output_path} exists after save. Last modified: {time.ctime(mod_time_after)} (Timestamp: {mod_time_after})"
            )
