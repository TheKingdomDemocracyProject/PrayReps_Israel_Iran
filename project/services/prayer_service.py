...
def get_queued_representatives(limit=None):
    ...
    conn = None
    if not DATABASE_URL:
        current_app.logger.error("DATABASE_URL not set, cannot fetch queued representatives.")
        return items
    try:
        conn = get_db_conn()
        with conn.cursor(cursor_factory=DictCursor) as cursor:
            ...
            current_app.logger.debug(f"Fetched {len(items)} 'queued' representatives (PostgreSQL).")
    except psycopg2.Error as e:
        current_app.logger.error(f"PostgreSQL error in get_queued_representatives: {e}")
    except Exception as e_gen:
        current_app.logger.error(f"Unexpected error in get_queued_representatives: {e_gen}", exc_info=True)
    finally:
        if conn:
            conn.close()
    return items
...
def get_prayed_representatives(country_code=None):
    ...
    conn = None
    if not DATABASE_URL:
        current_app.logger.error("DATABASE_URL not set, cannot fetch prayed representatives.")
        return items
    try:
        conn = get_db_conn()
        with conn.cursor(cursor_factory=DictCursor) as cursor:
            ...
            rows = cursor.fetchall()
            items = [dict(row) for row in rows]
            current_app.logger.debug(f"Fetched {len(items)} 'prayed' representatives (country: {country_code or 'all'}) (PostgreSQL).")
    except psycopg2.Error as e:
        current_app.logger.error(f"PostgreSQL error in get_prayed_representatives: {e}")
    except Exception as e_gen:
        current_app.logger.error(f"Unexpected error in get_prayed_representatives: {e_gen}", exc_info=True)
    finally:
        if conn:
            conn.close()
    return items
...
def mark_representative_as_prayed(candidate_id):
    ...
    conn = None
    if not DATABASE_URL:
        current_app.logger.error("DATABASE_URL not set, cannot mark representative as prayed.")
        return None, 0
    ...
    try:
        ...
        with conn.cursor(cursor_factory=DictCursor) as cursor:
            ...
            cursor.execute("SELECT * FROM prayer_candidates WHERE id = %s AND status = 'queued'", (candidate_id,))
            item_to_update_row = cursor.fetchone()

            if not item_to_update_row:
                current_app.logger.warning(f"Attempted to mark item ID {candidate_id} as prayed, but it was not found or not in 'queued' state (PostgreSQL).")
                return None, 0
            ...
            if rows_affected > 0:
                conn.commit()
                current_app.logger.info(f"Marked representative ID {candidate_id} as 'prayed' (PostgreSQL).")
                ...
                processed_item_details['timestamp'] = now_timestamp.strftime('%Y-%m-%d %H:%M:%S')
                return processed_item_details, rows_affected
            else:
                conn.rollback() # Should not happen if initial check passed
                current_app.logger.warning(f"Mark as prayed for ID {candidate_id} (PostgreSQL) affected 0 rows, despite initial check.")
                return None, 0
    except psycopg2.Error as e:
        current_app.logger.error(f"PostgreSQL error marking representative ID {candidate_id} as prayed: {e}")
        if conn:
            conn.rollback()
        return None, 0
    except Exception as e_gen:
        current_app.logger.error(f"Unexpected error in mark_representative_as_prayed (PG): {e_gen}", exc_info=True)
        if conn:
            conn.rollback()
        return None, 0
    finally:
...
def put_representative_back_in_queue(candidate_id, new_hex_id=None):
    ...
    conn = None
    if not DATABASE_URL:
        current_app.logger.error("DATABASE_URL not set, cannot put representative back in queue.")
        return 0
    ...
    try:
        conn = get_db_conn()
        with conn.cursor(cursor_factory=DictCursor) as cursor:
            cursor.execute("SELECT * FROM prayer_candidates WHERE id = %s AND status = 'prayed'", (candidate_id,))
            item_to_update_row = cursor.fetchone()

            if not item_to_update_row:
                current_app.logger.warning(f"Attempted to put item ID {candidate_id} back in queue (PG), but it was not found or not in 'prayed' state.")
                return 0
            ...
            if rows_affected > 0:
                conn.commit()
                current_app.logger.info(f"Put representative ID {candidate_id} back to 'queued' (PG), hex_id set to {final_hex_id}.")
            else:
                conn.rollback() # Should not happen
                current_app.logger.warning(f"Put back for ID {candidate_id} (PG) affected 0 rows.")
            return rows_affected
    except psycopg2.Error as e:
        current_app.logger.error(f"PostgreSQL error putting representative ID {candidate_id} back to queue: {e}")
        if conn:
            conn.rollback()
        return 0
    except Exception as e_gen:
        current_app.logger.error(f"Unexpected error in put_representative_back_in_queue (PG): {e_gen}", exc_info=True)
        if conn:
            conn.rollback()
        return 0
    finally:
...
def get_available_hex_id_for_country(country_code, exclude_candidate_id=None):
    ...
    if hex_map_gdf is None or hex_map_gdf.empty or 'id' not in hex_map_gdf.columns:
        current_app.logger.warning(f"Hex map data or 'id' column not available for {country_code} via current_app.hex_map_data_store. Cannot assign hex_id.")
        return None
    ...
    try:
        ...
        current_app.logger.error(f"PostgreSQL error fetching used hex_ids for {country_code}: {e}")
        return None # Cannot determine available hex_ids
    except Exception as e_gen:
        current_app.logger.error(f"Unexpected error in get_available_hex_id_for_country (PG db part): {e_gen}", exc_info=True)
        return None
    finally:
        if conn:
            conn.close()
    ...
    if not available_hex_ids:
        current_app.logger.warning(f"No available hex_ids to assign in {country_code} (PG).")
        return None

    random.shuffle(available_hex_ids)
    assigned_hex_id = available_hex_ids.pop()
    current_app.logger.info(f"Assigned available hex_id {assigned_hex_id} for {country_code} (PG).")
...
def purge_all_data():
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            conn.commit()
            current_app.logger.info(f"Purged all {cursor.rowcount} items from prayer_candidates table (PostgreSQL).")
        return True
    except psycopg2.Error as e:
        ...
        current_app.logger.error(f"Unexpected error in purge_all_data (PG): {e_gen}", exc_info=True)
        if conn:
            conn.rollback()
        return False
    finally:
...
def get_party_statistics(country_code):
    ...
    prayed_items_for_country = get_prayed_representatives(country_code) # This now uses PostgreSQL
    party_counts = {}
    ...
    current_app.logger.debug(f"Calculated party statistics for {country_code} (PG): {sorted_party_counts}")
    ...
    for item in items_for_timedata:
        ...
        if ts_str:
            ...
            if country_code == 'overall':
                ...
                value_detail['country'] = target_countries.get(item_country_code, {}).get('name', 'Unknown')
            values.append(value_detail)

    current_app.logger.debug(f"Fetched timedata for {country_code} (PG): {len(timestamps)} entries.")
...
def get_overall_prayed_count():
    ...
    conn = None
    if not DATABASE_URL:
        current_app.logger.error("DATABASE_URL not set, cannot get overall prayed count.")
        return count
    try:
        conn = get_db_conn()
        with conn.cursor() as cursor: # No DictCursor needed for simple count
            cursor.execute("SELECT COUNT(id) FROM prayer_candidates WHERE status = 'prayed'")
            row = cursor.fetchone()
        ...
        current_app.logger.error(f"Unexpected error in get_overall_prayed_count (PG): {e_gen}", exc_info=True)
    finally:
        if conn:
            conn.close()
    return count
