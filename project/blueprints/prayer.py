...
def process_item_htmx():
    ...
    if processed_item_details and rows_affected > 0:
        current_app.logger.info(f"Successfully processed item ID {item_id_to_process} as prayed.")
        ...
        map_image_path_updated = url_for('static', filename='hex_map.png') + f"?v={datetime.now().timestamp()}"

        next_item_to_display = prayer_service.get_next_queued_representative()

        total_possible_in_csvs = 0
        for cc_config_iter in current_app.config['COUNTRIES_CONFIG']: # Renamed loop variable
            df = prayer_service.fetch_csv_data(cc_config_iter)
            num_to_select = current_app.config['COUNTRIES_CONFIG'][cc_config_iter].get('total_representatives', len(df))
        ...
        current_item_html = render_template("partials/_current_item_display.html",
                                            current=next_item_to_display)
        map_html = render_template("partials/_map_image_display.html",
                                   map_image_path=map_image_path_updated)
        stats_summary_html = render_template("partials/_stats_summary.html",
        ...
        response_html = render_template("partials/_current_item_display.html", current=next_item_to_display)

        # Add OOB swap elements for map and stats
        response_html += render_template("partials/_map_image_display.html", map_image_path=map_image_path_updated, _oob_swap_id="map-image-container")
        response_html += render_template("partials/_stats_summary.html", remaining=current_remaining, queue_size=queue_size, total_prayed_overall=prayed_count_overall, _oob_swap_id="stats-summary-container")

        return response_html, 200

    current_app.logger.warning(f"Failed to process item ID {item_id_str} or item not found/already processed.")
...
def put_back_htmx():
    ...
    current_app.logger.info(f"HTMX request to put back item ID: {candidate_id_str} (Country: {country_code_form})")
    ...
    if not country_code_form or country_code_form not in current_app.config['COUNTRIES_CONFIG']:
        current_app.logger.error(f"Invalid or missing country_code '{country_code_form}' in put_back_htmx request.")
    ...
    if rows_affected > 0:
        current_app.logger.info(f"Successfully put item ID {candidate_id} back in queue. New hex_id (if any): {new_hex_id_to_assign}")

        prayed_list_for_country_updated = prayer_service.get_prayed_representatives(country_code=country_code_form)

        current_queue_items_for_map = prayer_service.get_queued_representatives()
        map_service.generate_country_map_image(country_code_form, prayed_list_for_country_updated, current_queue_items_for_map)
        ...
        for item_iter in prayed_list_for_country_updated: # Renamed loop variable
            ...
            item_copy['formatted_timestamp'] = format_pretty_timestamp(item_copy.get('timestamp'))
            party_name_from_log = item_copy.get('party', 'Other')
            party_data = country_party_info.get(party_name_from_log, country_party_info.get('Other', other_party_default))
            item_copy['party_class'] = party_data['short_name'].lower().replace(' ', '-').replace('&', 'and')
            item_copy['party_color'] = party_data['color']
            prayed_items_display.append(item_copy)

        updated_list_html = render_template("partials/_prayed_list_table.html",
    ...
    current_app.logger.warning(f"Failed to put item ID {candidate_id_str} back in queue or item not found/already queued.")
...
def get_queue_json():
    ...
    for item_iter in items: # Renamed loop variable
        if 'country_code' in item_iter and item_iter['country_code'] in current_app.config['COUNTRIES_CONFIG']:
            item_iter['country_name'] = current_app.config['COUNTRIES_CONFIG'][item_iter['country_code']]['name']
        else:
...
def prayed_list_page_html(country_code):
    if country_code == 'overall':
        ...
        for item_iter in prayed_items_all: # Renamed loop variable
            ...
            if item_country_code and item_country_code in current_app.config['COUNTRIES_CONFIG']:
                 display_item['country_name_display'] = current_app.config['COUNTRIES_CONFIG'][item_country_code]['name']
            else:
                 display_item['country_name_display'] = 'Unknown Country'
            display_item['formatted_timestamp'] = format_pretty_timestamp(item_iter.get('timestamp')) # Use original item for timestamp
        ...
        current_app.logger.warning(f"Invalid country code '{country_code}' for prayed list page. Redirecting to default.")
        default_country = list(current_app.config['COUNTRIES_CONFIG'].keys())[0]
        return redirect(url_for('prayer.prayed_list_page_html', country_code=default_country))
    else:
        prayed_items_for_country = prayer_service.get_prayed_representatives(country_code=country_code)
        current_country_party_info = current_app.config['PARTY_INFO'].get(country_code, {})
        other_party_default = {'short_name': 'Other', 'color': '#CCCCCC'}
        ...
        for item_original_iter in prayed_items_for_country: # Renamed loop variable
            ...
            item['formatted_timestamp'] = format_pretty_timestamp(item_original_iter.get('timestamp')) # Use original item for timestamp
...
def prayed_list_default_redirect():
    if current_app.config['COUNTRIES_CONFIG']:
        ...
        return redirect(url_for('prayer.prayed_list_page_html', country_code=default_country_code))
