...
def generate_map_for_country_direct(country_code):
    ...
    if hex_map_gdf is None or hex_map_gdf.empty:
        current_app.logger.error(
            f"Map data (GeoDataFrame) not available for {country_code} in direct generation."
        )
    ...
    return (
        jsonify(
            status=f"Map directly generated for {country_code} at static/hex_map.png"
        ),
        200,
    )
...
def home():
    ...
    for country_code_iter in current_app.config[
        "COUNTRIES_CONFIG"
    ]:  # Changed variable name
        df = prayer_service.fetch_csv_data(country_code_iter)
        num_to_select = current_app.config["COUNTRIES_CONFIG"][country_code_iter].get(
    ...
    current_app.logger.debug(
        f"Home page: Displaying map for {map_to_display_country}. Current item: {current_item_display.get('person_name') if current_item_display else 'None'}"
    )
    ...
    map_image_path = (
        url_for("static", filename="hex_map.png") + f"?v={datetime.now().timestamp()}"
    )
    now = datetime.now()
...
def generate_map_for_country_json_route(country_code):
    ...
    map_image_path = (
        url_for("static", filename=map_image_filename)
    ...
    if success:
        ...
        current_app.logger.error(
            f"Map generation failed for {country_code}, serving placeholder path."
        )
        return (
            jsonify(
