...
def _populate_static_stores(app_instance):
    ...
    """
    app_instance.logger.info("Populating static data stores on app_instance (hex_map_data_store, deputies_data, etc.)...")

    # Ensure stores are initialized on app_instance (should be done in create_app)
    if not hasattr(app_instance, 'hex_map_data_store'): app_instance.hex_map_data_store = {}
    if not hasattr(app_instance, 'post_label_mappings_store'): app_instance.post_label_mappings_store = {}
    if not hasattr(app_instance, 'deputies_data'): app_instance.deputies_data = {}
    ...
    for country_code in COUNTRIES_CONFIG.keys(): # Use imported COUNTRIES_CONFIG
        ...
        if country_code not in app_instance.deputies_data:
            app_instance.deputies_data[country_code] = {'with_images': [], 'without_images': []}
        ...
        if not df_country.empty:
            ...
            process_deputies(df_country, country_code) # Call it, it will use current_app.deputies_data
            app_instance.logger.debug(f"Processed deputies for {country_code} (data on current_app).")
        ...
        if os.path.exists(map_path):
            ...
            app_instance.logger.debug(f"Loaded hex map for {country_code} onto app_instance.")
        else:
        ...
        if post_label_path and os.path.exists(post_label_path):
            app_instance.post_label_mappings_store[country_code] = load_post_label_mapping(post_label_path)
            app_instance.logger.debug(f"Loaded post label mapping for {country_code} onto app_instance.")
        else:
...
def initialize_application(app_instance): # app_instance is the Flask app from create_app
    ...
    app_instance.logger.info("Data Initializer: Static data stores populated on app_instance.")
    ...
    app_instance.logger.info("Data Initializer: Prayed-for data loading (current_app.prayed_for_data) attempted.")
